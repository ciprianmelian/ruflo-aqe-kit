...
- **Setup + proof are the entry point (SETUP-V1/PROOF-V1, Patch 59):** `ruflo-kit setup <target>` takes a fresh machine to a proved stack (prereqs → global installs → init → sync → opt-in brain KB → proof); its exit code IS the proof verdict, and it NEVER starts the daemon. `ruflo-kit proof <target>` = 13 disk-evidence probes run twice (pass 2 under `env -i`); `PROVED` only when both passes agree FAIL-free.
