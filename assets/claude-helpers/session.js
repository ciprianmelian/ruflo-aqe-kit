...
// Monotonic id source: Date.now() alone collides when start() is called twice
// within the same millisecond (each session must get a distinct id).
let lastSessionTs = 0;
function nextSessionId() {
  let ts = Date.now();
  if (ts <= lastSessionTs) ts = lastSessionTs + 1;
  lastSessionTs = ts;
  return `session-${ts}`;
}
