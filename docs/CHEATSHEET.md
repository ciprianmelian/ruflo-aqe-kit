...
- **LoRA is trained AND consumed (Patch 55):** `RUFLO-LORA-ADAPT-V1` adapts Router B's query embedding through the trained LoRA at route time (and persists `totalAdaptations`). If `verify-learning` #3 reports `totalAdaptations=0` again, the seam was wiped by an upgrade — re-run `fix-ruflo`.
- **Current global versions:** ruflo **v3.32.7**, agentic-qe **3.12.2** (as of 2026-07-18). aqe 3.12.2 migrations (`qe_trajectories.metadata_json` + `qe_pattern_nulls`) applied; ~530 trajectories intact (point-in-time).
