...
| `proof` → `lib/proof.sh` | 15 disk-evidence probes (PROOF-V1, Patch 59; P14–P15 Patch 61), run **twice** — pass 2 re-executes under `env -i` so it can only see committed disk state. `PROVED` iff both passes have zero FAILs and identical verdicts; a probe that flips = `UNSTABLE`. P13 requires the `sqlite3` CLI and says so when it's missing (Patch 65). |
...
- **Current global versions:** ruflo **v3.32.8**, agentic-qe **3.13.0** (as of 2026-07-20; aqe drifted 3.12.2→3.13.0 — `fix-aqe`'s dynamic anchors still land, the dream-lockfix chunk hash moved again `XNNYHQLW → J3L52EA4`, adoption-verified on rust-r8n). aqe 3.12.2 migrations (`qe_trajectories.metadata_json` + `qe_pattern_nulls`) applied earlier; ~530 trajectories intact (point-in-time).
- **`sqlite3` CLI is a real prerequisite for honest probes** (Patch 65): without it, P13 formerly mis-read healthy stores as "locked/unwritable with no live holder" (now it FAILs with the true cause), and verify-learning silently under-detects (graded `partial` where the truth was HOLLOW). S1 warns when it's absent — install it (`apt install sqlite3` / ships with macOS) before trusting a proof verdict.
