...
| `fix-brain` → `lib/fix-brain.sh` | Register the **MCP-only** `search_ruvnet` knowledge-brain (`BRAIN-MCP-V1`, Patch 53) in `.mcp.json` — a 1.7 GB Ed25519-verified KB at `~/.cache/ruvnet-brain/kb`. `--download` fetches the KB. **No hooks, no launchd, no plugin** → zero always-on cost. Idempotent; re-run is a health probe. |
...
- **Current global versions:** ruflo **v3.32.2**, agentic-qe **3.12.2** (as of 2026-07-17). aqe 3.12.2 migrations (`qe_trajectories.metadata_json` + `qe_pattern_nulls`) applied; 531 trajectories intact.
