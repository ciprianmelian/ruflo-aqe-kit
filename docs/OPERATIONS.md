...
| `CLAUDE.md` | stale `@claude-flow/cli@latest` refs rewritten; Runtime section appended (only if the `## Agentic QE v3` anchor exists); marker-delimited kit-adoption note appended at the end (anchor-independent; hand-written `ruflo-aqe-kit` notes honored, not duplicated — `KIT-ADOPTION-NOTE-V1`, Patch 66) | `.fixruflo.bak`, auto-restore on failed verification; prose untouched by design |
...
**Recommended sequence — the one-verb path (Patch 66):**

1. `bin/ruflo-kit status <target> --forensics` — read-only; see what the kit is walking into, including the manual-install fingerprints (shell history, daemon state files, stale npx caches) that tell you what "manual install" actually meant here.
2. `bin/ruflo-kit adopt <target> --dry-run` — previews the snapshot plan and the whole setup/fix cascade with zero writes.
3. `bin/ruflo-kit adopt <target>` — snapshot (WAL-safe backup + per-table row-count baseline) → `setup` (adopt REFUSES `--force` by design) → recount → printed before→after preservation diff. Exit 0 = proof PROVED **and** no baselined table shrank (`MEMORY-PRESERVE-PROOF-V1`); exit 2 = a table shrank. Re-check the diff anytime with `adopt --verify-only`.
4. Restart Claude Code so the MCP/settings changes load; the daemon stays opt-in.

**Granular alternative** (what `adopt` composes, if you want each step under your own eyes):

1. `bin/ruflo-kit status <target>` — read-only baseline view.
2. `bin/ruflo-kit sync <target> --dry-run` — the fix cascade prints its plan with zero writes; in dry-run each stage's counter reads `N would-change(s)` (the count of its `[dry-run] Would:` lines — Patch 66).
3. `bin/ruflo-kit snapshot <target>` — the receipt: WAL-safe `.backup` of the stores + row-count manifest to `~/.ruflo-kit/backups/<target>-<ts>/` (never `cp` a live WAL DB — §Safety). Also git-commit `.mcp.json` / `CLAUDE.md` / `.claude/` if the target is a repo.
4. `bin/ruflo-kit setup <target>` (or `init` + `sync`) — **without `--force`**. Init no-ops the existing ruflo pieces, adds the missing layers; sync converges configs; the 16-probe proof verifies (exit code = verdict).
5. `bin/ruflo-kit adopt <target> --verify-only` — recount vs the snapshot baseline; the preservation table is your zero-losses proof.
6. Restart Claude Code so the MCP/settings changes load; the daemon stays opt-in.
...
**The adoption record in the target's `CLAUDE.md` is now automated** (`KIT-ADOPTION-NOTE-V1`, fix-ruflo Step 5j, Patch 66 — so it runs on every `adopt`/`setup`/`sync`). Unlike the Runtime append (which needs the `## Agentic QE v3` anchor and never fires on hand-written files), Step 5j appends a **marker-delimited, kit-managed block to the end of the target's `CLAUDE.md`** regardless of anchors: kit clone path, `sync`/`status`/`proof` as the heal/check verbs, the deliberate states a future operator must NOT "fix" (hoisted-vs-nested AgentDB shadow pin, global-binary MCP launch, 3-channel daemon-off), and the "re-run `sync` after any global reinstall" rule. Content between the markers is re-asserted to canonical on every run; a pre-existing hand-written note mentioning `ruflo-aqe-kit` (like the adopted target's) is honored, not duplicated; a target with no `CLAUDE.md` is skipped. Two things the block does NOT carry, so note them yourself if they matter: the backup snapshot location (it's in `~/.ruflo-kit/backups/…/manifest.json` and the `.claude-flow/data/adoption-baseline.json` pointer) and any target-specific operational quirks. Without the record, the next operator or agent session in the target re-does exactly the manual actions adoption just repaired (npx MCP registration, "fixing" the pin mismatch).

**Field note — first real memory-preserving adoption (a Rust workflow-platform target, Ubuntu host, 2026-07-20; Patch 65).** The sequence above was executed end-to-end against a manually-installed target carrying ~11k learning rows. What generalizes:

- **Forensics first:** shell `history` named the exact manual actions to undo (npx MCP registration, `ruflo init --wizard --force`, repeated `doctor --fix`) — read it before assuming what "manual install" means.
- **Take the baseline yourself:** WAL-safe `.backup` of both stores + per-table row counts BEFORE any kit verb, re-count after — "zero losses, only additions" is then a verifiable claim, not an assurance. (No `sqlite3` CLI? `better-sqlite3` from the global ruflo install exposes the same online-backup API.) *Since Patch 66 this is exactly what `snapshot` + `adopt` do for you, better-sqlite3 fallback included (`MEMORY-PRESERVE-PROOF-V1` / `KIT-SQLITE-SHIM-V1`).*
- **Stray daemons predate the gates:** `daemonAutoStart: true` had manufactured 4 auto-spawned daemons (one per cwd, incl. `$HOME` and `ui/` subdirs). Adoption never kills a RUNNING daemon by policy — but auto-spawned strays cache the OLD dist and will negate fresh dist patches, so kill what nobody deliberately started (`pgrep -f "cli.js daemon"`) before `setup`.
- **HOLLOW is the honest verdict for pre-kit history, and it clears in three moves:** `harvest` consumes the backlog (3,895 experiences → LoRA train + episodes/skills), but `totalAdaptations` stays 0 — the 868 pre-kit session records accrued with no consumption seam installed — so proof still FAILs until **one live route runs from the target cwd** (`ruflo hooks route -t "..."`) and applies the adapter (`totalAdaptations` 0→1 in `.swarm/lora-weights.json`). Sequence: harvest → one route → proof PROVED. *Since Patch 66, `fix-learning` step 12 runs that one priming route automatically when it finds a trained-but-unapplied adapter (`LORA-APPLY-PRIME-V1`, gated on the `RUFLO-LORA-ADAPT-V1` dist sentinel).*
- **Version drift is survivable:** the target's global aqe was 3.13.0 (ahead of kit docs); `fix-aqe`'s INSERT-anchor lookup still found the dream-lockfix chunk (hash moved again, `→ J3L52EA4`).
- **Instrument prereq:** the host had no `sqlite3` CLI — P13 mis-FAILed healthy stores and verify-learning under-graded (`partial` masking a real HOLLOW) until it was installed. P13 now names the true cause; treat S1's sqlite3 warning as a hard prereq for trusting verdicts.

**Verify:** `bin/ruflo-kit proof <target>` ends PROVED, `bin/ruflo-kit status <target>` shows the pre-existing learning stores with their prior row counts intact — and your own before/after row-count diff shows no table shrank.
...
**What `proof` actually proves (16 probes, disk/real-output only — never MCP self-reports):** ruflo + aqe CLIs and MCP `initialize` handshakes; the three AgentDB slots (standalone/nested exactly at the pin, hoisted at-or-past the upstream floor); ≥23 controller classes importable from the nested slot; better-sqlite3 loads; brain registered (KB optional = WARN); statusline renders; dist sentinels n/N; `verify-learning` ∈ {live, partial} (its probe #11 `SEAM-SENTINEL-V1` — Patch 64 — additionally FAILs the loop verdict if the installed dist lost the `SONA-TRAIN-V1`/`RUFLO-LORA-ADAPT-V1` seams to an upstream bump, the silent write-only-revert case the older probes could not see); health parses sanely (the comma-bug tripwire); `hooks route` answers; all three learning stores accept a write lock (a store held by a LIVE writer such as the session's aqe-mcp = WARN "not assessable", never FAIL; a missing `sqlite3` CLI = FAIL naming that cause, not a phantom "locked with no holder" — Patch 65); **P14 daemon-gates** (FAIL if `claude-flow.config.json` lacks `daemon.autostart:false` or the installed statusline lacks the `DAEMON-AUTOSTART-3-V1` pin; a *running* daemon is only a WARN — a deliberate start is the operator's right); **P15 statusline-truth** (renders the TARGET's INSTALLED `.claude/helpers/statusline.cjs` — the kit's own asset is used only as a fresh-target fallback when nothing is installed yet, Patch 71 F6 fix — and cross-checks its `--json` against an independent sqlite recount + a real regex test-count); **P16 memory-roundtrip** (`MEMORY-ROUNDTRIP-V1`, `kit_memory_roundtrip_check` in `lib/common.sh`: a real `ruflo memory` store → CLI retrieve → **independent on-disk read** via the sqlite3 CLI or the node better-sqlite3 fallback → purge → post-purge retrieve, entirely inside a disposable `mktemp -d` — every other memory/store check in this file only proves a store exists, holds rows, or accepts a momentary write lock, never that a value written through the CLI actually comes back out through a reader independent of that same CLI; catches the exact shape where a store echoes a cached value back on retrieve while the real disk write silently no-ops. The independent read is mandatory to reach PASS, not best-effort: a host with neither the sqlite3 CLI nor a loadable global better-sqlite3 FAILs the round trip as a gap (detail says "no independent verification performed" explicitly, never a silent healthy) rather than skipping the check the way an earlier build of this probe briefly did (fixed, B19 — regression-tested in `tests/proof-memory-roundtrip-probe.test.js`). FAILs, not WARNs, when there is nothing to assess — `ruflo` missing from PATH already hard-FAILs P1 ruflo-cli, so this never invents a new way for an otherwise-healthy target to lose PROVED); **P9 sentinels** also drives `HOOK-BLOCK-EXIT2-V1` behaviorally when a hook-handler is installed — a real `pre-bash` invocation with a recognized dangerous-command payload must exit 2, not just a sentinel-string grep (Patch 71 F5 fix — a sentinel that survives in unreachable code, or a hook-handler regenerated without the patch, now FAILs instead of passing silently); it also asserts, for every installed ruvector `bin/mcp-server.js` copy found under the same global-root search fix-ruflo's `RUVECTOR-EXECSAFE-V1` patcher uses, that zero shell-interpolated `execSync(` calls remain — a PROPERTY check (`dist_defect_present`), not a sentinel-string grep, so a partially-patched or reverted copy that still carries a stray patch comment still FAILs; zero copies installed is healthy (not a dependency here), never a FAIL. It runs the whole set **twice** — the second pass re-executed under a scrubbed environment (`env -i`) so nothing from the calling shell can influence it — and only identical clean passes earn `PROVED` (exit 0). A probe that changes its answer between passes yields `UNSTABLE`. **F3 escalation (Patch 71):** P2/P3/P12 tag a WARN with `[noresp-timeout]` only when a REGISTERED component never proved a working handshake — covering both "invoked, no valid answer within timeout" AND "could not even be spawned" (e.g. ENOENT from a stale `.mcp.json` path after an nvm root switch — round 2 closed this on P3, the one place it was left untagged; P2's equivalent branch was already a hard FAIL) — never for an absent-by-design component or a benign empty result. Escalation requires the verdict (WARN) AND the marker's presence to match on BOTH passes — the free-text remainder of the detail may differ; a probe marked on only one pass does not escalate (an intentional, conservative non-escalation, not a gap). When it does match on both, the x2 verdict escalates that probe to `FAILED` — a persistent no-answer/no-spawn is no longer indistinguishable from first-run warmup.

**Fresh targets PROVE honestly (Patches 62–63, first fresh-target e2e 2026-07-18):** kit verbs pin `AQE_PROJECT_ROOT=<target>` unconditionally (`AQE-ROOT-INHERIT-GUARD-V1`) so running `setup` from a shell that lives in ANOTHER project — e.g. a Claude session in the kit repo, whose settings export that project's pin — can no longer poison the target's `aqe init`. A zero-history target grades the learning loop **"primed", not hollow**: HOLLOW fires only on real evidence (unharvested eligible `captured_experiences`, or ≥1 session record with the LoRA adapter never applied at inference). The loop then closes on real work: a live Claude Code session in the target captures experiences (CLI runs do NOT) → `ruflo-kit harvest <target>` replays them into episodes/skills (`HARVEST-VECLESS-V1`: embedding-less rows — all of them on aqe 3.12.2 — harvest to the reflexion sink; no fabricated training vectors) → verify-learning flips to populated/engaged → PROVED with substance. **Target-owner note (corrected — was stale):** the previous wording here ("kit init vendors an upstream ruflo tree under `.agents/skills/` (~19.5k of its own tests)") described pre-#2777 behavior and is no longer true on the installed dist. As of the installed ruflo, `init`'s `maybeInstallSkillsSh()` (`@claude-flow/cli/dist/src/commands/init.js`) writes a single small `SKILL.md` platform descriptor at `.agents/skills/ruflo/SKILL.md` — not a vendored test tree — and detects/cleans up a bloated leftover from an older version (checking for markers like `Cargo.toml`/`crates`/`.git`/`docs`/`plugins` or an on-disk size over a small budget) before re-materializing just that one file. A target that was `init`ed by an older, pre-#2777 kit may still carry the old bloated tree until it is re-initialized; re-check the current behavior by reading `maybeInstallSkillsSh()` and its bloat-detection comment in the installed dist rather than trusting a specific size/count here, since both are upstream's to change.
