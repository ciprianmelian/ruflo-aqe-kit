...
**Record the adoption in the target's `CLAUDE.md`.** If the target's `CLAUDE.md` is hand-written (no `## Agentic QE v3` anchor), the kit's automatic Runtime append never fires — add a short, dated "kit-managed" bullet block to the target's own operational-status section instead: kit clone path, `sync`/`status`/`proof` as the heal/check verbs, the deliberate states a future operator must NOT "fix" (hoisted-vs-nested AgentDB shadow pin, global-binary MCP launch, 3-channel daemon-off), the "re-run `sync` after any global reinstall" rule, and the backup snapshot location. Without this, the next operator or agent session in the target re-does exactly the manual actions adoption just repaired (npx MCP registration, "fixing" the pin mismatch).
...
- **Take the baseline yourself:** WAL-safe `.backup` of both stores + per-table row counts BEFORE any kit verb, re-count after — "zero losses, only additions" is then a verifiable claim, not an assurance. (No `sqlite3` CLI? `better-sqlite3` from the global ruflo install exposes the same online-backup API.)
